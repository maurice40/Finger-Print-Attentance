...
public class GlobalData {
    ...
    public UserItem FindUserItemByFp(byte[] fpdat) {
        for (int i = 0; i < userList.size(); i++) {
            if (userList.get(i).template1.length() > 512) {
                byte[] ref = Base64.decode(userList.get(i).template1, Base64.DEFAULT);
                if (FPMatch.getInstance().MatchTemplate(fpdat, ref) > 60) {
                    return userList.get(i);
                }
            }
            if (userList.get(i).template2.length() > 512) {
                byte[] ref = Base64.decode(userList.get(i).template2, Base64.DEFAULT);
                if (FPMatch.getInstance().MatchTemplate(fpdat, ref) > 60) {
                    return userList.get(i);
                }
            }
    ...
    public void AppendRecordCsv(RecordItem rs) {
        RecordFile.AppendToFileCsv(sDir + "/recordslist.csv", rs);
    }
    ...
    public void ClearRecordsList() {
        ...
        RecordFile.ReCreate(sDir + "/recordslist.csv");
    }
